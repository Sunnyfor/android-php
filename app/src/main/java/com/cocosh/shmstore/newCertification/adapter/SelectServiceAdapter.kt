 ...
 */
class SelectServiceAdapter(lsit: ArrayList<SelectServiceModel>) : BaseRecycleAdapter<SelectServiceModel>(lsit) {

    var index = -1

    override fun onBindViewHolder(holder: BaseRecycleViewHolder, position: Int) {
        holder.getView<TextView>(R.id.sheng).text = getData(position).operatorName
        if (position == index) {

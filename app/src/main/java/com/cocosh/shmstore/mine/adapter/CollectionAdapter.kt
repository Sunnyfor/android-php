 ...
 * 收藏
 */
class CollectionAdapter(var mContext: Context, list: ArrayList<CollectionModel>) : BaseRecycleAdapter<CollectionModel>(list) {
    override fun onBindViewHolder(holder: BaseRecycleViewHolder, position: Int) {
        holder.itemView.name.text = getData(position).redPacketName
        if (getData(position).redPacketMoneyStatus == "1") {
        ...
        if (getData(position).isGive == "1") {
            holder.itemView.status.text = "已赠送"
            holder.itemView.status.setBackgroundResource(R.color.grayBtn)
        }

        holder.itemView.number.text = "红包总金额:￥" + getData(position).redPacketMoney
        GlideUtils.loadFullScreen(mContext, getData(position).advertisementBanner, holder.itemView.pic)
    }

...
class MessageSystemAdapter(var mContext: Context, list: ArrayList<MsgModel>) : BaseRecycleAdapter<MsgModel>(list) {
    ...
    override fun onBindViewHolder(holder: BaseRecycleViewHolder, position: Int) {
        ...
        if (getItemViewType(position) == 2) {
            holder.itemView.tvTimeImage.text = getData(position).createDate
            holder.itemView.tvTitleImage.text = getData(position).title
            holder.itemView.tvDescImage.text = getData(position).details
            GlideUtils.load(mContext, getData(position).resourcesUrl, holder.itemView.ivImage)
        }
        if (getItemViewType(position) == 3) {
            holder.itemView.tvTimeDraw.text = getData(position).createDate
...
//                MoneyWaterActivity.start(mContext, Constant.TYPE_MY)
    ...
    override fun getItemViewType(position: Int): Int {
        if (list[position].messageType == MessageType.TEXT_MESSAGE.type) {
            return 1
        }
        if (list[position].messageType == MessageType.GRAPHIC_MESSAGE.type) {
            return 2
        }
        if (list[position].messageType == MessageType.PUT_FORWARD_LAUNCH_MESSAGE.type) {
            return 3
        }
        if (list[position].messageType == MessageType.PUT_FORWARD_TO_ACCOUNT_MESSAGE.type) {
            return 4
        }
        return 5
    }
